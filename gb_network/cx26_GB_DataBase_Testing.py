...
#%cd /data/wesley/Connexin_Energy_Network_Analysis/cx26_GB_Network_Analysis

networkDataDir='GB_BTW_Network_Data'
...
for iTable,edgeTableName in enumerate(pbar):
    pbar.set_description_str(
        '%05g '%(iTable)+'.'.join(list(map(
            lambda x: x.split('__')[-1],
            edgeTableName.split('.')[1:4]
        ))))
        
    tempFrame=pd.read_csv(networkDataDir+'/'+edgeTableName)
...
conn=create_connection(network_database_directory+'/'+network_database_name)
...
for iTable,nodeTableName in enumerate(pbar):
    pbar.set_description_str(
        '%05g '%(iTable)+'.'.join(list(map(
            lambda x: x.split('__')[-1],
    ...
    tempFrame=pd.read_csv(networkDataDir+'/'+nodeTableName)
...
visStruc=pt.load('/data/visualization_structure.pdb',top='/data/visualization_structure.parm7')
...
for iTable,nodeTableName in enumerate(pbar):
    ...
    if (len(tables)>=nTables) or (iTable==(len(pbar)-1)):
